...
def _cross_references_query() -> str:
    return """
        SELECT
            selected.group_id AS "group_id",
            selected.input_id AS "input_id",
...
def _relations_query(direction: Literal["outgoing", "incoming", "both"]) -> str:
    predicates: list[str] = []
...
def _hierarchy_query(*, reverse: bool) -> str:
    if reverse:
...
class _SelectionState:
    ...
    candidate: pl.DataFrame | None = None
    matches: pl.DataFrame | None = None
    invalid_targets: frozenset[str] | None = None
...
class ChEBICompoundSelection:
    ...
    def compounds(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            schema=_relation_schema(columns),
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_compounds_query(),
                schema=_relation_schema(columns),
                request=request,
                required={"compound"},
                operation="extract compounds",
            ),
            is_pure=True,
        )
    ...
    def names(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                ...
                operation="extract compound names",
            ),
            is_pure=True,
        )
    ...
    def cross_references(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_cross_references_query(),
                schema=_relation_schema(columns),
                request=request,
                required={"compound_cross_reference"},
                operation="extract compound cross-references",
            ),
            is_pure=True,
        )
    ...
    def relations(
        ...
        *,
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_relations_query(direction),
                schema=_relation_schema(columns),
                request=request,
                required={"compound_relation"},
                operation="extract compound relations",
            ),
            is_pure=True,
        )
    ...
    def structures(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_structures_query(),
                schema=_relation_schema(columns),
                request=request,
                required={"compound_structure"},
                operation="extract compound structures",
            ),
            is_pure=True,
        )
    ...
    def wurcs(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                ...
                schema=_relation_schema(columns),
                request=request,
                required={"compound_wurcs"},
                operation="extract compound WURCS",
            ),
            is_pure=True,
        )
    ...
    def ancestors(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_hierarchy_query(reverse=False),
                schema=_relation_schema(columns),
                request=request,
                required={"compound_relation"},
                operation="traverse compound hierarchy",
            ),
            is_pure=True,
        )
    ...
    def descendants(self) -> pl.LazyFrame:
        ...
        return register_replayable_source(
            ...
            batches=lambda request: _iter_joined_batches(
                snapshot,
                query=_hierarchy_query(reverse=True),
                schema=_relation_schema(columns),
                request=request,
                required={"compound_relation"},
                operation="traverse compound hierarchy",
    ...
    def _candidates(self) -> pl.DataFrame:
        with self._state.lock:
            if self._state.candidate is not None:
                return self._state.candidate.clone()
        ...
        self._state.candidate = frame
        return frame.clone()

    def _invalid_target_inputs(self) -> frozenset[str]:
        with self._state.lock:
            cached = self._state.invalid_targets
        ...
        with duckdb.connect(str(publication.path), read_only=True) as connection:
            ...
            if "validation_issue" not in metadata_tables:
                ...
                self._state.invalid_targets = result
                return result
            _create_input_table(connection, self._input_rows)
            rows = connection.execute(
                """
...
def _iter_frame_batches(
    ...
    *,
    ...
    if request.columns is not None:
        columns = [name for name in request.columns if name in schema]
        if columns:
            frame = frame.select(columns)
    yield from _frame_slices(frame, request.effective_batch_size)


def _iter_joined_batches(
    selection: ChEBICompoundSelection,
    *,
    query: str,
    schema: SchemaDict,
    request: _RelationScanRequest,
    ...
    selection._require_tables(required, operation)  # pyright: ignore[reportPrivateUsage]  # paired selection boundary
    matches = selection._eager_matches()  # pyright: ignore[reportPrivateUsage]  # paired selection boundary
    if matches.is_empty():
        return
    publication = selection.database._require_publication()  # pyright: ignore[reportPrivateUsage]  # paired query boundary
    ...
    try:
        _create_selected_table(
            connection,
            matches,
            grouped=selection._is_grouped,  # pyright: ignore[reportPrivateUsage]
        )
        requested = (
            None
        ...
        if requested is None:
            ...
            query_to_run = f"SELECT {projection} FROM ({query}) AS _bioextract_relation"
        result = connection.execute(query_to_run)
        reader = _chebi_arrow_reader(result, request.effective_batch_size)
...
def _chebi_arrow_reader(result: Any, batch_size: int) -> Any:
